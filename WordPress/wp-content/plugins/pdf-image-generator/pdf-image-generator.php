...
class PIGEN {
	...
	public function pigen_activate() { // Check the server whether or not it has imageMagick enabled.
		$verify_imagick = $this->pigen_imageMagick_ver();
		if ( $verify_imagick ) {
			exec( 'which gs', $gs_arr, $gs_res ); 
			if ( $gs_res !== 0 ){
				$verify_imagick = $this->pigen_imagick_ver();
				if ( $verify_imagick === false ) {
		...
		if ($options['action'] == 'update' && $options['type'] == 'plugin' ){
			foreach($options['plugins'] as $each_plugin){
				if ($each_plugin == $current_plugin_path_name ){
					...
					if ( strpos( $verify_imagick, 'imageMagick' ) !== false ) $verify_imagick = $this->pigen_imageMagick_ver();
					elseif ( strpos( $verify_imagick, 'imagick' ) !== false ) $verify_imagick = $this->pigen_imagick_ver();
	...
	public function pigen_imageMagick_ver() {
		$imageMagick = false;
		if ( function_exists('exec') ) {
			exec( 'convert -version', $arr, $res );
			if ( $res === 0 && count($arr) > 2 ) {
				preg_match('/ImageMagick ([0-9]+\.[0-9]+\.[0-9]+)/', $arr[0], $v);
				if( version_compare($v[1],'6.7.5') < 0 ) $imageMagick = 'imageMagick_lt675';
				else $imageMagick = 'imageMagick';
			}
		}
		return $imageMagick;
	}
	...
	public function pigen_imagick_ver() { 
		$imagick = false;
		if( extension_loaded('imagick') ) {
			$im = new imagick();
			$v = $im->getVersion();
			preg_match('/ImageMagick ([0-9]+\.[0-9]+\.[0-9]+)/', $v['versionString'], $v);
			if( version_compare($v[1],'6.3.8') < 0 ) $imagick = 'imagick_lt638';
			else $imagick = 'imagick';
		}
		return $imagick;
	}
	...
	public function pigen_options() { 
		...
		if ( isset($_GET['run']) ) {
			...
			echo
				...
				//"\t\t\t".'<p class="subfield note">'.__( 'A color specification by name, hexadecimal color code.', 'pdf-image-generator' ).'</p>'."\n".		
				"\t\t".'</div>'."\n";
			...
			$verify_imagick = ( isset($opt['verify_imagick']) ? $opt['verify_imagick'] : 'imageMagick' );
			$imageMagick_val = $this->pigen_imageMagick_ver();
			$imagick_val = $this->pigen_imagick_ver();
			if ( strpos( $verify_imagick, 'imageMagick' ) !== false ) $verify_imagick = $imageMagick_val;
			elseif ( strpos( $verify_imagick, 'imagick' ) !== false ) $verify_imagick = $imagick_val;
			echo
				"\t\t".'<div>'."\n".
				...
				"\t\t".'</div>'."\n".
				"\t\t".'<div>'."\n".
				"\t\t\t".'<p><span class="float"><input type="radio" id="pigen_verify_imageMagick" name="pigen_verify_imagick" value="'.$imageMagick_val.'" '.( $verify_imagick === $imageMagick_val ? 'checked="checked"' : '' ).( $imageMagick_val ? '' : ' disabled' ).' /><label for="pigen_verify_imageMagick">'.__( 'Use imageMagick with exec function', 'pdf-image-generator' ).'</label></span><span class="float"><input type="radio" id="pigen_verify_imagick" name="pigen_verify_imagick" value="'.$imagick_val.'" '.( $verify_imagick === $imagick_val ? 'checked="checked"' : '' ).( $imagick_val ? '' : ' disabled' ).' /><label for="pigen_verify_imagick">'.__( 'Use imagick PHP Extension', 'pdf-image-generator' ).'</label></span></p>'."\n".
				"\t\t".'</div>'."\n".
				"\t".'</fieldset>'."\n".
		...
		echo 
			...
			'<script type="text/javascript">jQuery(function($) { '.
				...
				// 	'if ( $(this).is(":checked") ) $( this ).closest( "div" ).children( ".subfield" ).find( "input, select, textarea" ).removeAttr( "disabled" ); '.
				// 	'else $( this ).closest( "div" ).children( ".subfield" ).find( "input, select, textarea" ).attr( "disabled", "disabled" ); '.
				// '}).trigger( "keyup" ); '.
			'});</script>'."\n";
	}
	...
	public function pigen_generate( $file ){ // Generate thumbnail from PDF
		...
		if ( $verify_imagick == 'imagick' || $verify_imagick == 'imagick_lt638' ) {
			try { 
				$imagick = new imagick();
				if ( $property ) {
			...
			if ( $verify_imagick == 'imageMagick' ) $alphaoff = "-alpha remove"; else $alphaoff = "-flatten";
			if ( $property ) {
				if ( $image_bgcolor !== 'white' && preg_match( '/^#[a-f0-9]{6}$/i', $image_bgcolor ) ) $image_bgcolor = "\"$image_bgcolor\"";
				$imageMagick = "convert -density 150 -set units PixelsPerInch -density 72 -quality {$quality} -background {$image_bgcolor} {$alphaoff} -resize {$max_width}x{$max_height} {$file}[0] {$file_url}";
			} else {
				$imageMagick = "convert -density 72 -background white {$alphaoff} {$file}[0] {$file_url}";
			}
			//シンボリックリンク作成
			// $target = '/usr/bin/gs';
			// $link = '/usr/local/bin/gs';
			// symlink($target, $link);
	...
	public function pigen_insert( $html, $attach_id, $attachment ) { // Insert thumbnail instead of PDF
		if( $attach_id && get_post_mime_type ( $attach_id ) === 'application/pdf' ){
			...
			if ( $size === 'url' ){
				...
				$attach_output = '<img src="'. $attach_image[0] .'" alt="'.get_post_meta( $attach_image_id, '_wp_attachment_image_alt', true ).'" width="'. $attach_image[1] .'" height="'. $attach_image[2] .'" class="'.( $align ? 'align' .$align. ' ' : '' ). 'size-' .esc_attr( $size ). ' wp-image-'.$attach_image_id.'" />';
...
<?php }
	...
	function pigen_save_post( $post_id ){
